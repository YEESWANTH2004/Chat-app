  ...
  return (
    <div className='min-h-screen bg-cover flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler}
       className='border-2 bg-white/8 text-white border-gray-500 p-6 px-10 flex flex-col gap-6 rounded-lg shadow-lg'>
...
{ currState === 'Sign up' && !isDataSubmitted && (
  ...
  <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 text-white border-none text-sm font-light py-2 px-20 rounded-full cursor-pointer'>
    ...
    <div className='flex items-center gap-2 text-sm font-gray-400'>
      <input type="checkbox" />
      <p>Agree to terms of use & privacy policy.</p>
    </div>

    <div className='flex flex-col gap-2'>
      {currState === 'Sign up' ? (<p className='text-sm text-gray-500'>Already have an account? 
        <span onClick={() => {
          setCurrState('Login')
          setIsDataSubmitted(false)}}
         className='font-medium text-violet-500 cursor-pointer px-3'>Login here</span></p> 
        ) : (<p className='text-sm text-gray-500 flex text-center justify-center gap-3'>Create an account!! <span onClick={() => {
